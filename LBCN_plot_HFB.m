...
%               pre_defined_bad:bad indicies generated by exclude pchan
%               function (HFOs and spikes). Matrix size = data size before
%               TF downsampling 
%   Output:     HFB Signals.
%   -----------------------------------------
%   =^._.^=     Su Liu
...
if ~exist('signal_all','var')
    ...
    for N=1:length(cellstr(evtfile))
        ...
        [sig, sig2,nanidx, spec, spec2] = compute_HFB(data, fs, type, fbands, atf_check, ...
            bc_type,window_bc ,pre_defined, window, dsamp, linspace(0,information.f,19));
        cn = size(data,1);
        ...
        for nn=1:Nt
            for cc=1:cn
                ...
                for jj=1:numel(total_raw)
                    ...
                    currtf2 = cat(3, currtf2, total_tf2{jj}{nn}{cc});
                end
                all_plot{nn}{cc}=currcond;
                all_plot2{nn}{cc}=currcond2;
                all_nan{nn}{cc}=currcond3;
                %all_tf{nn}{cc}=nanmean(currtf(:,:,~all(isnan(all_nan{nn}{cc}))),3);
                all_tf{nn}{cc}=nanmean(currtf2(:,:,~all(isnan(all_nan{nn}{cc}))),3);
            end
        end
    else
    ...
    beh_fp=all_plot;
       ...
       fprintf('------ Saving tfmaps ... \n');
        save(fullfile(dir_curr,strcat('Epoched_tfmap','.mat')),'all_tf','-v7.3');
