function [D,nanid] = LBCN_epoch_bc(fname,evt,indc,fieldons,twepoch,bc,fieldbc,twbc,prefix)
...
%     Dold = events(Dold, i, select_events(D.events, ...
%         [time(D,trl(i, 1))  time(D,trl(i, 2))]));
...
%-Save new evoked M/EEG dataset; use filtered D for plotting
%--------------------------------------------------------------------------
D = Dnew;
% D2 = Dold;
save(D);
