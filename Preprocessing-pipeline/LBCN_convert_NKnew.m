...
for id = 1:size(fname,1)
    ...
    for i =1:length(list)
        ...
        if strfind(nel,'DC') % All DC channels in one file
            ...
            eeglab = [eeglab,{elecnam}];%channel labels
            ielec=[ielec;i];
            labelecog = [labelecog,list{i}];
        end
    end
    %Dw = D;    
    labelecog_bck=labelecog;
    ...
    if numel(slab) ~= numel(labs)
        ...
        group = find(diff(numrepet)~=1);
        group=[group; length(numrepet)];
        group1 = [1;group+1];
        group1(end) =[];
        namegroup=[group1 group];
        for j = 1:size(namegroup,1)
            
            idrep = numrepet(namegroup(j,1):(namegroup(j,2)));
            idrep = [idrep ; idrep(end)+1];
            for jj=1:length(idrep)
                labs{idrep(jj)} = strcat(labs{idrep(jj)}(1:end-4),'_',num2str(jj),labs{idrep(jj)}(end-3:end));
            end
        end
        list = labs;
        eeglab = {};
...
%                 listlabel(ii) = list{ii};
%              end
%              
%              indrepet = find(strcmp(listlabel,labrepet));
...
%      end
    ...
    % Create ECoG file
     prefix = 'ECoG_';
     crc_eeg_rdata_edf(deblank(fname(id,:)),labelecog_bck,prefix,path_block);
