function D = crc_eeg_rdata_edf(Fdata,chanlist,prefix,path)
% converts EEG data from EDF- to SPM12-format
%
% FORMAT D = crc_eeg_rdata_edf(Fdata)
% 
% Fdata       - filename of edf-file
% chanlist    - list of channels to load
% D           - SPM12 compatible matlab structure for an M/EEG object.
%
...
% Written by Y. Leclercq & C. Phillips, 2008. J. Schrouff 2015
...
% $Id$
%%%%%%%Modified by Su Liu 2018 to correct duplicate channel names
...
[fdata,header] = crc_eeg_readHeaderEDF(Fdata,chanlist,prefix,pth);
...
for ii=1:Nchannels
    ...
    tempchan.label = deblank(chan_names(ii,:));
