function [fdata,header] = crc_eeg_readHeaderEDF(fname,chanlist,prefix,path)
...
% $Id$
%Modified by Su Liu 2018 @LBCN, Stanford

if nargin < 1
    fname = spm_select(1, 'any', 'Select EDF file');
end;

[fdata,header] = readedf(fname,chanlist,prefix,path);
...
function [fdata,header] = readedf(filename,chanlist,prefix,path)
...
%%%%%%%%%%check for duplicate labels and rename%%%%%%%%%%%%%%%%%%%
try
labs = cellstr(header.channelname) ;   
[slab,ilab,~] = unique(labs);
    if numel(slab) ~= numel(labs)
        ...
        for j = 1:length(sameid)
            header.channelname(sameid(j),1:length(ls{j})) = ls{j};
        end
    end
catch
...
if nargin>1 && ~isempty(chanlist)
    [itokeep] = ismember(cellstr(header.channelname),chanlist);
else
    itokeep = 1:header.channels;
end
chtk = find(itokeep);
...
if var_SR
    ...
    fdata = fullfile(path,[prefix,fname,'.dat']);
    fo = fopen(fdata,'w');
...
if numel(chtk)~= header.channels
    header.channels = numel(chtk);
    header.channelname = header.channelname(chtk,:);
    header.transducer = header.transducer(chtk,:);
    header.physdime = header.physdime(chtk,:);
    header.physmin = header.physmin(chtk,:);
    header.physmax = header.physmax(chtk,:);
    header.digimin = header.digimin(chtk,:);
    header.digimax = header.digimax(chtk,:);
    header.prefilt = header.prefilt(chtk,:);
    header.nrsamples = header.nrsamples(chtk,:);
end
