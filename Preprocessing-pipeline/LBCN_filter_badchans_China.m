...
% Updated by A. Kucyi, 04/11/2017 to filter out 50Hz line noise (instead of 60Hz) for data
% from China
...
for i= 1:numel(ibadchans)
    nbad = find(ismember(listbad,ibadchans{i}));
    if ~isempty(nbad)
        countbad(nbad) = countbad(nbad)+1;
    end
    nbadfile = ~ismember(ibadchans{i},listbad);
    if ~isempty(find(nbadfile))
        listbad = [listbad,ibadchans{i}(find(nbadfile))];
        countbad = [countbad;ones(length(find(nbadfile)),1)];
    end
end
    
if ~conserv
    totbad = union(chanlabels(d{end},bchfile2),listbad);
elseif conserv>0
    totbad = listbad(find(floor(countbad/(conserv*length(ibadchans)))));
