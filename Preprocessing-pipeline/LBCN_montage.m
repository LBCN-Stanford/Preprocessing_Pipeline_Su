...
for i = 1:size(files,1)
    ...
    label = changelabels(1:D.nchannels,D.chanlabels);
    D = chanlabels(D, 1:D.nchannels, label);
    ...
    if strcmpi(option, 'av_good') || strcmpi(option, 'average')
        ...
        montage.labelorg  = changelabels(1:length(eegchan),montage.labelorg );
        montage.labelnew    = D.chanlabels(eegchan);
        montage.labelnew  = changelabels(1:length(eegchan),montage.labelnew );
