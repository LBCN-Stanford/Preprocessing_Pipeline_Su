...
%       currpath:   full directory of the mat file.
...
if isempty(onsetind)
    ...
    for i =1:size(dm,2)
        ...
        if length(ind) < 20
            thr = thr-0.5;
            ind = find(adif2 > thr*std(abs(dif2(:,i))));
        elseif length(ind) > 100
            thr = thr+0.5;
            ind = find(adif2 > thr*std(abs(dif2(:,i))));
        end
        ind = ind+3;
        ...
        spi = find(abs(yy-2000)==min(abs(yy-2000)));
        [~,in] = max(hh(spi:end));
        interv = yy(in+spi-1);        
...
if length(stimchan)>3
   ...
   id = max(abs(dm)) == max(max(abs(dm))); % channel with largest amplitude
   %[~,id] = max(varall);
   cond = strcat('stim ',chanLabel{stimchan(id)});
end
for i =1:size(df3,2)
    for j = 1:size(group,1)
            clear alligned
...
%            if strcmpi(groupname{k},'ekg')
%                continue;
%            else
            try
            plotchan = grpplot1(k):grpplot2(k);
                plot_CCEP
                print(gcf, fullfile(pth,strcat('CCEP_',fn,'_',groupname{k})),'-opengl','-r300','-dpng');
                close;
            catch
                continue;
            end
%           end
    ...
    plot_CCEP
    print(gcf, fullfile(pth,strcat('CCEP_ALL_',fn)),'-opengl','-r300','-dpng');
    close;
end
