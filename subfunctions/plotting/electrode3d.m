function [H,H2,H3]=electrode3d(lx,ly,lz,color,msize,showslice,type,plot,hc,ax)
%load the template and get the norms;
% Modified from the original MRI CT coregistration software
% Written by Su Liu @ LBCN 2018
...
for i=1:N
    ...
    if type==4
        ...
        if type==5
            ...
            zz=z+cz;
            zz=[z(1,:);z(2,:)*1]+cz;
            Pr=rot3d([xx(:),yy(:),zz(:)],[cx cy cz],rotParam(i,1:3),rotParam(i,4));
            ...
            Pr=rot3d([xx2(:),yy2(:),zz2(:)],[cx cy cz],rotParam(i,1:3),rotParam(i,4));
            xx2=reshape(Pr(:,1),e,e);
            yy2=reshape(Pr(:,2),e,e);
            zz2=reshape(Pr(:,3),e,e);
            x2=[x2 xx2 na2];
...
for i = 1:N
    if plot
        H=surf(ax,X,Y,Z,'facecolor',color(i,:),'LineStyle','none','facealpha',alpha);
        set(ax,'nextplot','add');
        H2=surf(ax,x2,y2,z2,'facecolor',color(i,:),'LineStyle','none','facealpha',alpha);
        if type==4
            H3=surf(ax,XS,YS,ZS,'facecolor',[0 0 0],'facealpha',0.3,...
                'LineStyle','none');
        else
            H3=[];
        end
