function [V,elecMatrix]=plot_ele_slices(cfg,elecId)
...
% Load MRI
mriFname='/Volumes/Mimi/LBCN/Data/Race/Stanford/TW118/elec_recon/brainmask.nii';%fullfile(fsdir,fsSub,'mri','brainmask.mgz');
if ~exist(mriFname,'file')
   error('File %s not found.',mriFname); 
end
mri.vol= spm_read_vols(spm_vol(mriFname));
...
if isempty(mgridFname)
    [elecMatrix, elecLabels, elecRgb]=mgrid2matlab('/Volumes/Mimi/LBCN/Data/Race/Stanford/TW118/elec_recon/S17_118.mgrid');
end
...
%    if depthElecs(elecId)
        ...
        yStart=.01;
        ht=.4;
        set(gca,'position',[xStart yStart wdth ht]);
        D1=squeeze(mri.vol(:,xyz(elecId,2),:));
        ...
        D2=squeeze(mri.vol(xyz(elecId,1),:,:));
        ...
        D3=squeeze(mri.vol(:,:,xyz(elecId,3)));
...
%             ht=text([formattedLabel '; mgrid coords(' num2str(elecMatrix(elecId,:)-1) '); fsurf coords(' num2str(xyz(elecId,:)) '); ' anatLabel], ...
%                 'title');
%             set(ht,'fontsize',16,'fontweight','bold','color',[1 1 1]);
%         else
%             tt=strcat([formattedLabel '; Anatomical Location: ' anatLabel], ...
%                 'title');
%             ht=text(tt);
