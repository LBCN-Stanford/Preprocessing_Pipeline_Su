function [D,Ddiod] = LBCN_convert_NKnew_rename(fname, path_save,downsample,exclude)
...
%Updated by Su Liu: rename same labels
...
for id = 1:size(fname,1)
    ...
    %%%%%%%%%%%Check that all channels have different labels, if not,
    %%%%%%%%%%%rename the labels
    labelecog_bck=labelecog;
    
    labs(ielec) = labelecog;
    labs(idc) = labeldc;
    ...
    if ~all
        if iscell(exclude)
            ...
            labelecog_bck = labelecog_bck(itk);
            eeglab = eeglab(itk);
            ielec = ielec(itk);
        end
    end
