function [H,H2,H3]=scatter3d(lx,ly,lz,color,msize,alpha,type,plot,hc)
if nargin<7
    type=1;
    plot=1;
end
...
for i=1:N
    ...
    if type==4
        ...
        if type==5
            if tp(3)<cz
                angle=-1.1*(pi/2+asin((tp(3)-cz)/norm(tp-[cx cy cz])));
            else
                angle=-1.1*(-pi/2+asin((tp(3)-cz)/norm(tp-[cx cy cz])));
            end                                              
        else
...
%         Y=[Y y*rr+cy na];
%         h=z+1.05;
%         x2=[x2 rt.*cos(t)+cx na2];
%         y2=[y2 rt.*sin(t)+cy na2];
%         z2=[z2 (cz+1)*ones(size(rt)) na2];
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
if plot==1
